...
- Update the [Route Matrix](../references/authoring-contract.md#route-matrix-nine-scenarios).
...
- Cover all nine scenarios: baseline path, branch path, repair cycle, human checkpoint, cold resume, stale or out-of-scope or invalid artifact, contradictory suggested route losing to evidence, narrower executor substitution with a stable actor-like gate boundary, and premature completion rejected by the terminal guard.
- Repair only mismatches, then repeat prediction and derivation for the repaired case.

## Checkable Completion State

- All nine scenarios contain prediction, derived route, and match result.
